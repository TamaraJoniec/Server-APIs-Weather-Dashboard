...
async function locationURL() {
  ...
  try {
    const response = await fetch(
      `https://api.openweathermap.org/geo/1.0/direct?q=${search.toLowerCase()}&limit=5&appid=${apiKey}`
    );
    ...
    .then((data) => {
      ...
      currentWeatherElement.innerHTML = `
          <div>
            ...
            <p>${new Date(data.dt * 1000).toLocaleDateString(
              "en-GB",
              options
            )}</p>
          </div>
  ...
  fetch(
    ...
    .then((data) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        forecastWind.textContent = `Wind: ${data.list[i].wind.speed.toFixed(
          0
        )} MPH`;
